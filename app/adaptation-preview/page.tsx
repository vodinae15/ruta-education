...
export default function AdaptationPreviewPage() {
  ...
  // Пример распределения основного текстового контента по блокам (с Markdown форматированием)
  const mainTextBlock1 = `## Что такое химические реакции?

Химические реакции окружают нас повсюду: от процессов в нашем организме до явлений природы и технологических процессов. **Понимание механизмов химических реакций** является фундаментом для изучения многих естественных наук.

**Химическая реакция** — это процесс превращения одних веществ *(реагентов)* в другие *(продукты реакции)*. При этом происходит:

- Разрыв старых химических связей
- Образование новых связей
- Перераспределение атомов между молекулами

> Важно: В ходе химической реакции атомы не исчезают и не появляются, а лишь перераспределяются между молекулами.`

  const mainTextBlock2 = `## Классификация химических реакций

Все химические реакции можно классифицировать по различным признакам. Один из важнейших критериев — это **энергетический баланс реакции**.

### Экзотермические реакции

Экзотермические реакции сопровождаются **выделением энергии** в виде тепла. Примеры:

- **Горение**: когда дрова сгорают в камине, выделяется большое количество тепла и света
- **Нейтрализация**: реакция кислоты со щелочью

### Эндотермические реакции

Эндотермические реакции, напротив, требуют постоянного **подвода энергии** извне.

Классический пример — *фотосинтез* в растениях, где энергия солнечного света используется для превращения углекислого газа и воды в глюкозу и кислород.`

  const mainTextBlock3 = `## Скорость химической реакции

**Скорость химической реакции** — это изменение концентрации реагентов или продуктов за единицу времени. На скорость реакции влияют несколько факторов:
...
4. **Присутствие катализатора** — вещества, которые ускоряют реакцию, не расходуясь при этом`
  ...
  const mainTextBlock4 = `## Роль катализаторов

**Катализаторы** играют особую роль в химии. Они снижают *энергию активации* реакции — минимальную энергию, необходимую для начала химического процесса.

### Ферменты в живых организмах

В живых организмах роль катализаторов выполняют **ферменты** — белковые молекулы, которые ускоряют биохимические реакции в *миллионы раз*.

> Без ферментов большинство реакций в организме протекали бы слишком медленно для поддержания жизни.

### Применение в промышленности

В промышленности катализаторы применяются повсеместно:
- Производство серной кислоты
- Переработка нефти
- Создание полимеров`

  const mainTextBlock5 = `## Закон сохранения массы

**Закон сохранения массы**, сформулированный М.В. Ломоносовым, гласит:

> Масса веществ, вступивших в реакцию, равна массе образовавшихся продуктов.

Этот закон является основой для составления уравнений химических реакций.
