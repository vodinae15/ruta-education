...

{
  "block1": {
    ...
    "structuredText": {
      ...
      "mainText": "Основной текст блока в Markdown формате (300-600 символов):\\n\\n## Заголовок\\n\\nТекст с **жирным**, *курсивом*, списками:\\n- Пункт 1\\n- Пункт 2\\n\\n> Важная цитата или заметка"
    },
    "media": [
      ${allMediaElements.length > 0 ? `
      {
        "type": "image|video|audio",
        "url": "URL из исходного контента (скопируй ТОЧНО)",
        "caption": "Краткое описание медиа"
      }
      ` : '// Пусто, если нет медиа в блоке'}
    ]
  }
}
...
- mainText: основной текст с Markdown (##, **, *, -, >)
- Сохраняй ВСЕ факты, примеры, детали из исходного контента

ПРАВИЛА РАБОТЫ С МЕДИА:
- Скопируй URL медиа-элементов ТОЧНО как в исходном контенте
- Распредели медиа по блокам логично:
  - Изображения/диаграммы → в блок, где они упоминаются
  - Видео → обычно в блок 2 или 4
  - Аудио → если есть, в блок 2 или 3
- Добавь краткое caption для каждого медиа

РАСПРЕДЕЛЕНИЕ КОНТЕНТА ПО БЛОКАМ:
- БЛОК 1: Введение и обзор (3-4 карточки + 2-3 секции)
- БЛОК 2: Основная теория (4-6 карточек + 3-4 секции)
- БЛОК 3: Практические примеры (3-4 карточки + 2-3 секции)
- БЛОК 4: Углубленное изучение (4-5 карточек + 2-3 секции)
- БЛОК 5: Итоги и проверка (3-4 карточки + 2 секции)

КРИТИЧЕСКИ ВАЖНО:
1. НЕ добавляй информацию, которой нет в исходном контенте
2. НЕ изменяй факты, даты, имена, термины
3. Создавай карточки ТОЛЬКО из терминов исходного контента
...

{
  "block1": {
    ...
    "mainText": "Основной текст блока в Markdown (300-600 символов с объяснением темы)",
    "media": [
      {
        "type": "image|video|audio",
        "url": "URL из исходного контента (скопируй ТОЧНО)",
  ...
  "block2": {
    "comparisonTable": {
      "rows": [
        ...
        // минимум 3-4 строки на блок
      ]
    },
    "mainText": "Основной текст блока в Markdown (300-600 символов)",
    "media": [...]
  }
}
...
РАСПРЕДЕЛЕНИЕ КОНТЕНТА ПО БЛОКАМ:
- БЛОК 1: Обзор темы + Mermaid схема (6-9 узлов)
- БЛОК 2: Основная теория + таблица понятий (4-5 строк)
- БЛОК 3: Практические примеры + таблица примеров (3-4 строки)
- БЛОК 4: Углубленное изучение + таблица деталей (3-4 строки)
- БЛОК 5: Итоги + итоговая таблица (3-4 строки)

КРИТИЧЕСКИ ВАЖНО:
1. НЕ добавляй информацию, которой нет в исходном контенте
2. НЕ изменяй факты, даты, имена, термины
3. Создавай таблицы ТОЛЬКО из понятий исходного контента
4. Копируй URL медиа ТОЧНО как есть
5. Верни JSON для ВСЕХ 5 блоков
...
Верни ТОЛЬКО валидный JSON без комментариев:
{
  "block1": { "mermaidDiagram": {...}, "mainText": "...", "media": [...] },
  "block2": { "comparisonTable": {...}, "mainText": "...", "media": [...] },
  "block3": { "comparisonTable": {...}, "mainText": "...", "media": [...] },
  "block4": { "comparisonTable": {...}, "mainText": "...", "media": [...] },
  "block5": { "comparisonTable": {...}, "mainText": "...", "media": [...] }
}
`
...
РАСПРЕДЕЛЕНИЕ КОНТЕНТА ПО БЛОКАМ:
- БЛОК 1: Введение + 3-4 аудио-карточки с основными терминами
- БЛОК 2: Основная теория + 4-5 аудио-карточек
- БЛОК 3: Практические примеры + 3-4 аудио-карточки
- БЛОК 4: Углубленное изучение + 3-4 аудио-карточки
- БЛОК 5: Итоги + AudioUploadBlock (задание на запись аудио-пересказа)

КРИТИЧЕСКИ ВАЖНО:
1. НЕ добавляй информацию, которой нет в исходном контенте
2. НЕ изменяй факты, даты, имена, термины
3. Создавай аудио-карточки ТОЛЬКО из терминов исходного контента
4. Копируй URL медиа ТОЧНО как есть
5. Верни JSON для ВСЕХ 5 блоков
...
Верни ТОЛЬКО валидный JSON без комментариев:
{
  "block1": { "audioCards": [...], "mainText": "...", "media": [...] },
  "block2": { "audioCards": [...], "mainText": "...", "media": [...] },
  "block3": { "audioCards": [...], "mainText": "...", "media": [...] },
  "block4": { "audioCards": [...], "mainText": "...", "media": [...] },
  "block5": { "audioUploadBlock": {...}, "mainText": "...", "media": [...] }
}
`
...

{
  "block1": {
    ...
    "mainText": "Основной текст блока в Markdown (300-600 символов с обзором темы)",
    "media": [...]
  }
}
  ...
  "block2": {
    "practicalText": {
      ...
      "tasks": [
        ...
        // минимум 2-3 задания на блок
      ],
      "criteria": "Критерии оценки выполнения заданий"
    },
    "mainText": "Основной текст блока в Markdown (300-600 символов с теорией)",
    "media": [...]
  }
}
...
- tasks: массив из 2-3 практических заданий
...
РАСПРЕДЕЛЕНИЕ КОНТЕНТА ПО БЛОКАМ:
- БЛОК 1: Цели обучения (3-5 целей) + обзор темы
- БЛОК 2: Основная теория + практические задания (2-3 задания)
- БЛОК 3: Применение на практике + задания с примерами (2-3 задания)
- БЛОК 4: Углубленная практика + сложные задания (2-3 задания)
- БЛОК 5: Итоговый проект + финальное задание (1-2 задания)

КРИТИЧЕСКИ ВАЖНО:
1. НЕ добавляй информацию, которой нет в исходном контенте
2. НЕ изменяй факты, даты, имена, термины
3. Создавай цели и задания ТОЛЬКО на основе исходного контента
4. Копируй URL медиа ТОЧНО как есть
5. Верни JSON для ВСЕХ 5 блоков
...
Верни ТОЛЬКО валидный JSON без комментариев:
{
  "block1": { "goalsChecklist": {...}, "mainText": "...", "media": [...] },
  "block2": { "practicalText": {...}, "mainText": "...", "media": [...] },
  "block3": { "practicalText": {...}, "mainText": "...", "media": [...] },
  "block4": { "practicalText": {...}, "mainText": "...", "media": [...] },
  "block5": { "practicalText": {...}, "mainText": "...", "media": [...] }
}
`
