...
const getAdaptationPrompt = (studentType: string, lessonContent: LessonContent): string => {
  ...
  // Извлекаем практические задания автора из блоков
  // Ищем блоки с практикой по названию или типу
  const practiceBlocks = lessonContent.blocks.filter(block => {
    const title = (block.title || '').toLowerCase()
    const content = (block.content || '').toLowerCase()
    const type = (block.type || '').toLowerCase()
    return title.includes('практик') ||
           title.includes('задан') ||
           title.includes('упражн') ||
           type.includes('practice') ||
           type.includes('task') ||
           content.includes('задание:') ||
           content.includes('выполните:') ||
           content.includes('практическое задание')
  })

  const authorPracticeContent = practiceBlocks.length > 0
    ? practiceBlocks.map(b => `${b.title || 'Практика'}:\n${b.content}`).join('\n\n')
    : ''
...
${authorPracticeContent ? `
ПРАКТИЧЕСКИЕ ЗАДАНИЯ ОТ АВТОРА:
${authorPracticeContent}

⚠️ КРИТИЧЕСКИ ВАЖНО: Автор УЖЕ предоставил практические задания! СОХРАНИ их ТОЧНО как есть в блоке 3 - НЕ изменяй формулировки, НЕ перефразируй, НЕ генерируй новые задания вместо них! Используй ДОСЛОВНО текст автора для практики!
` : ''}
...
- Добавь поле "practice": массив практических заданий
...
- ⚠️ ЕСЛИ выше указаны "ПРАКТИЧЕСКИЕ ЗАДАНИЯ ОТ АВТОРА" - используй ИХ ДОСЛОВНО! Не перефразируй, не изменяй!
- ЕСЛИ практических заданий от автора НЕТ - создай 3-5 заданий на основе материала из блоков 1-2
...
- ⚠️ ЕСЛИ выше указаны "ПРАКТИЧЕСКИЕ ЗАДАНИЯ ОТ АВТОРА" - используй ИХ ДОСЛОВНО! Не перефразируй, не изменяй!
- ЕСЛИ практических заданий от автора НЕТ - создай 3-5 заданий на основе материала из блоков 1-2
- AudioCards тоже добавляй для блока 3
...
- Каждый вопрос должен содержать:
  ...
  * "explanation": объяснение правильного ответа
- ОБЯЗАТЕЛЬНО создай 10 вопросов! Это критическое требование!
- AudioUploadBlock УБИРАЕМ - вместо него только test

РАСПРЕДЕЛЕНИЕ КОНТЕНТА ПО БЛОКАМ:
- БЛОК 1: Введение + 6-8 аудио-карточек + ПОЛНЫЙ текст
- БЛОК 2: Основная теория + 8-10 аудио-карточек + ПОЛНЫЙ текст
- БЛОК 3: Практическое закрепление + 6-8 аудио-карточек + ПОЛНЫЙ текст + 3-5 практических заданий
- БЛОК 4: Углубленное изучение с медиа + 6-8 аудио-карточек + ПОЛНЫЙ текст + ВСЕ медиа
- БЛОК 5: Итоги и тест + 6-8 аудио-карточек + ПОЛНЫЙ текст + 10 вопросов теста
...
Верни ТОЛЬКО валидный JSON без комментариев:
{
  ...
  "block3": { "introText": "...", "audioCards": [...], "mainText": "...", "practice": [...], "media": [] },
  "block4": { "introText": "...", "audioCards": [...], "mainText": "...", "media": [...] },
  "block5": { "introText": "...", "audioCards": [...], "mainText": "...", "test": [...], "media": [] }
}
`
...
ПРАВИЛА ДЛЯ БЛОКА 3 (ПРАКТИКА):
- Блок 3 - это ДОПОЛНИТЕЛЬНЫЕ ПРАКТИЧЕСКИЕ ЗАДАНИЯ для закрепления
- Добавь поле "practice": массив практических заданий
- Каждое задание должно содержать:
  * "id": уникальный идентификатор
  * "instruction": чёткая инструкция (что нужно сделать)
...
- ⚠️ ЕСЛИ выше указаны "ПРАКТИЧЕСКИЕ ЗАДАНИЯ ОТ АВТОРА" - используй ИХ ДОСЛОВНО! Не перефразируй, не изменяй!
- ЕСЛИ практических заданий от автора НЕТ - создай 3-5 заданий на основе материала из блоков 1-2
- PracticalText тоже добавляй для блока 3
...
- Каждый вопрос должен содержать:
  ...
  * "explanation": объяснение правильного ответа
- ОБЯЗАТЕЛЬНО создай 10 вопросов! Это критическое требование!
- PracticalText УБИРАЕМ для блока 5 - только test

РАСПРЕДЕЛЕНИЕ КОНТЕНТА ПО БЛОКАМ:
- БЛОК 1: Цели обучения (5-8 целей) + ПОЛНЫЙ обзор темы
- БЛОК 2: Основная теория + практические задания (3-5 заданий) + ПОЛНЫЙ текст
- БЛОК 3: Практическое закрепление + задания с примерами (3-5 заданий) + ПОЛНЫЙ текст + 3-5 дополнительных заданий (practice)
- БЛОК 4: Углубленная практика с медиа + сложные задания (3-5 заданий) + ПОЛНЫЙ текст + ВСЕ медиа
- БЛОК 5: Итоги и тест + ПОЛНЫЙ текст + 10 вопросов теста

КРИТИЧЕСКИ ВАЖНО:
1. НЕ добавляй информацию, которой нет в исходном контенте
2. НЕ изменяй факты, даты, имена, термины
3. Создавай МИНИМУМ 5-8 целей в блоке 1 и 3-5 заданий в блоках 2-3 (обязательно!)
4. Копируй URL медиа ТОЧНО как есть
5. Верни JSON для ВСЕХ 5 блоков
...
Верни ТОЛЬКО валидный JSON без комментариев:
{
  ...
  "block3": { "introText": "...", "practicalText": {...}, "mainText": "...", "practice": [...], "media": [] },
  "block4": { "introText": "...", "practicalText": {...}, "mainText": "...", "media": [...] },
  "block5": { "introText": "...", "mainText": "...", "test": [...], "media": [] }
}
`
