...
4. mainText (КРИТИЧНО - Основной текстовый материал блока):
   - Тип: строка с Markdown форматированием
   - Назначение: ПОРЦИЯ основного учебного текста для этого блока
   - Формат: Markdown (заголовки ##, списки, **жирный**, *курсив*, > цитаты)
   - Длина: 300-800 символов (2-4 абзаца)
   - Содержание:
     * Часть основного конспекта урока, распределённая на этот блок
     * Детальное объяснение концепций
     * Примеры и детали
     * Структурированный, хорошо отформатированный текст
   - ВАЖНО: Весь основной контент урока должен быть РАВНОМЕРНО распределён между mainText всех 5 блоков
`
    : `
...
4. mainText (КРИТИЧНО - Основной текстовый материал блока):
   ...
   - Содержание:
     * Часть основного конспекта урока, распределённая на этот блок
     * Детальное объяснение концепций из content.text
     * Примеры и детали
     * Структурированный, хорошо отформатированный текст
   - ВАЖНО: Весь основной контент урока должен быть РАВНОМЕРНО распределён между mainText всех 5 блоков
   - mainText ДОПОЛНЯЕТ адаптированный элемент, а не дублирует его
`
